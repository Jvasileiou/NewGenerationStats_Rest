...
public class TemplatePdf {
    ...
    private BasicPlayerAdapter teamAdapter1, teamAdapter2;
...
//    private double[] performanceTeamB = new double[6];
    ...
    public TemplatePdf(Context applicationContext, ArrayList<DataLiveActions> list,
                       BasicPlayerAdapter leftAdapter,
                       BasicPlayerAdapter rightAdapter,
...
//    }
    ...
    private void setTheRightTeam() {
        if (teamAdapter1.getTeam().getIsTeamA()) {
...
//                increaseForPerformanceTeamBSuccess(time, 1);
                if (action.contains("ΧΑΜΕΝΗ ΕΥΚΑΙΡΙΑ"))
...
//                            String useTeamB = nameTeamB;
    ...
    private void createFile() throws IOException {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            // For Android 10+ (Scoped Storage): Use public Download directory
            File downloadsDir = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS);
            fileDir = new File(downloadsDir, "NOT_DELETE/Matches");
            ...
            dest = new File(fileDir, nameTeamA.replace(",", "") + "-" +
                    nameTeamB.replace(",", "") + ".pdf").getAbsolutePath();

        } else {
            // For Android 9 and below: Use legacy external storage method (requires permissions)
            dest = Environment.getExternalStorageDirectory().toString() +
                    "/Download/NOT_DELETE/Matches/" +
                    nameTeamA.replace(",", "") + "-" +
                    nameTeamB.replace(",", "") + ".pdf";
            fileDir = new File(dest);
            fileDir.getParentFile().mkdirs(); // Ensure directory exists
        }
        ...
        // Handle font loading for the PDF
        String fontPath;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            // Use public directory for fonts on Android 10+
            fontPath = Environment
                    .getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS).toString() +
                    "/NOT_DELETE/Fonts/arialuni.ttf";
        } else {
            // Use legacy storage for Android 9 and below
            fontPath = Environment.getExternalStorageDirectory().toString() +
                    "/Download/NOT_DELETE/Fonts/arialuni.ttf";
        }

        // Initialize the font for the document
        normalFont = PdfFontFactory.createFont(fontPath, "Identity-H", true);
    }
    ...
    private void createHeaderTemplate() throws MalformedURLException {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            ...
            IMG = Environment
                    .getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS).toString() +
                    "/NOT_DELETE/logo.png";
            IMG_CARDS = Environment
                    .getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS).toString() +
                    "/NOT_DELETE/cards.png";
            IMG_CARD_YELLOW = Environment
                    .getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS).toString() +
                    "/NOT_DELETE/yellowcard.png";
            IMG_CARD_RED = Environment
                    .getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS).toString() +
                    "/NOT_DELETE/redcard.png";
            IMG_ATTACK_TEAM_A = Environment
                    .getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS).toString() +
                    "/NOT_DELETE/areaAttackTeamA.png";
            IMG_ATTACK_TEAM_B = Environment
                    .getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS).toString() +
                    "/NOT_DELETE/areaAttackTeamB.png";
